...
%Written by John Lesicko

% Additions for Transumral SALS O


% Create a class for SALSA. 
classdef SALSAnalysis2 <handle
    ...
    methods
        ...
        function this = SALSAnalysis
            ...
            %Override callback functions for objects in the figure
    ...
    methods (Access=private)
        ...
        function [OI,outAng,degree,corr,reconstruct,htens] = calcCentroid(Itheta,this)
            doFour = get(this.Figure.doFourth,'Value');
            if isequal(doFour,0)
        %% orig 2nd order code
                Htens=zeros(2);
        ...
        %% 4th order code
                ...
                reconstruct=((part1+part2+part3)*are);
            end
...
%             tic
            ...
            if colorchangeBool==1
                ...
                if colorval==1||colorval==0
                    this.colorMap=num2colormap(this.forOIColor,'jet',stats/100.0);
...
%                     hold on
%                     this.ietMult=str2num(get(this.Figure.ietMult,'String'));
%                     testM= processSomeData(this);
%                     axes(this.Figure.mainAxes);
%                     set(this.Figure.mainAxes,'XLim',[1 (this.input_size(1,2)*this.ietMult)]);
%                     set(this.Figure.mainAxes,'YLim',[1 (this.input_size(1,1)*this.ietMult)]);
%                     this.forIET=testM;
%     %                 this.forIET=100*rand(this.input_size(1,1)*this.ietMult,this.input_size(1,2)*this.ietMult);
%     %                 for i=1:1:this.input_size(1,1)*this.ietMult
%     %                     for j=1:1:this.input_size(1,2)*this.ietMult)
%     %                         this.forIET(i,j)=
%                     this.colorMap=num2colormap(this.forIET);
%                     this.img = image(this.colorMap);
%                     colormap(jet);
%                     colorbar;      
%                     hold off
...
%             set(this.Figure.mainAxes,'OuterPosition',[0,0,0.9,0.9]);
...
% %             px=1; py=1;
%             px=zeros(this.input_size(1,1),this.input_size(1,2)); py=zeros(this.input_size(1,1),this.input_size(1,2));
% %             px(:,:)=1;py(:,:)=1; 
...
%                if get(this.Pop.reconToggle,'Value')==1
% %                         this.baselineToggle=1;
%                         hold on
% %Plot reconstruction from tensors
%                         legend(this.Pop.iTheta,'Baseline')
%                         hold off
%                     else
%                         this.baselineToggle=0;
%                end
...
%            axis(this.Pop.iTheta);
%            plot(this.Pop.iTheta,drawthis1,'blue','LineWidth',2);
%            plot(this.Pop.iTheta,drawthis2,'green','LineWidth',2);
%            plot(this.Pop.iTheta,drawthis3,'red','LineWidth',2);
%            plot(this.Pop.iTheta,in180,(1:1000:65520),'--rs','LineWidth',2,...
%                 'MarkerFaceColor','r',...
%                 'MarkerSize',2);
%            plot(this.Pop.iTheta,this.forDegAng(this.V(2),this.V(1)),(1:1000:65520),':gs','LineWidth',2,...
%                 'MarkerFaceColor','g',...
%                 'MarkerSize',2);
%            plot(this.Pop.iTheta, this.forSkew(this.V(2),this.V(1)),(1:1000:65520),'--bs','LineWidth',2,...
%                 'MarkerFaceColor','b',...
%                 'MarkerSize',2);
%            set(this.Pop.iTheta,'YTickLabel',num2str(get(gca,'YTick').'))
%            hold off
...
%                     disp('Other');
        ...
        function outHist=buildHists(this,histType)
            ...
            if histType==1
                array=ceil(this.oiData_ODF*100);
...
%                 this.forGrayOUT=(temp+this.forGrayOUT)/2.0; %Method 1
                this.forGrayOUT=temp.*this.forGrayOUT; %Method 2
...
%                     export_fig(this.Figure.mainAxes,tempstr)
                    ...
                    axis off;
                    colorbar off;
%                     export_fig(this.Figure.mainAxes,tempstr,'-grey');
                    export_fig(this.Figure.mainAxes,tempstr,'-grey');
